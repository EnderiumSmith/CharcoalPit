...
public class ContainerCeramicPot extends Container{

	public final TileCeramicPot tile;
	
	public ContainerCeramicPot(IInventory player, TileCeramicPot tile) {
		this.tile=tile;
		for (int i = 0; i < 3; ++i)
        {
            for (int j = 0; j < 3; ++j)
            {
                this.addSlotToContainer(new SlotItemHandler(tile.items, j + i * 3, 62 + j * 18, 17 + i * 18));
            }
        }
